#> asset:mob/wither_skeleton/action/rapid_fire/
#
# shot arrow rapidly
#
...
# summon arrow ($difficulty_level=..4)
    execute if score $difficulty_level world matches 1..4 run function asset:mob/wither_skeleton/action/rapid_fire/summon_arrow with storage asset:mob __temp__

# summon arrow ($difficulty_level=5)
    execute if score $difficulty_level world matches 5.. run function asset:mob/wither_skeleton/action/rapid_fire/summon_arrow with storage asset:mob __temp__.0
    execute if score $difficulty_level world matches 5.. run function asset:mob/wither_skeleton/action/rapid_fire/summon_arrow with storage asset:mob __temp__.1
    execute if score $difficulty_level world matches 5.. run function asset:mob/wither_skeleton/action/rapid_fire/summon_arrow with storage asset:mob __temp__.2
...
# set data to summmoned arrow
    ...
    # set data
        # owner
            execute as @e[type=arrow,distance=..2,tag=rapid_fire.wither_skeleton.arrow,tag=!already_init] at @s run data modify entity @s Owner set from entity @n[tag=this] UUID
        # components
            execute as @e[type=arrow,distance=..2,tag=rapid_fire.wither_skeleton.arrow,tag=!already_init] at @s run data modify entity @s item.components set from entity @n[tag=this] equipment.offhand.components
        # weapon
            execute as @e[type=arrow,distance=..2,tag=rapid_fire.wither_skeleton.arrow,tag=!already_init] at @s run data modify entity @s weapon set from entity @n[tag=this] equipment.mainhand
    # add init tag
        tag @e[type=arrow,distance=..2,tag=rapid_fire.wither_skeleton.arrow,tag=!already_init] add already_init
