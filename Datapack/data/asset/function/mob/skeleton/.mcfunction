...
# swap weapon
    function asset:mob/skeleton/action/swap_weapon/check

# rapid fire
    # add score
        execute if score $difficulty_level world matches 1.. if entity @s[tag=mode.ranged] run scoreboard players add @s ai_timer.1 1
    # run
        execute if score @s ai_timer.1 matches 20 facing entity @n[tag=target] feet anchored eyes positioned ^ ^ ^ run function asset:mob/skeleton/action/rapid_fire/
    # reset score
...
# calcium toughness
    ...
    # run
        execute if score @s ai_timer.2 matches 1 run function asset:mob/skeleton/action/calcium_toughness/

# disgusting worm
    # add score
        execute if score $difficulty_level world matches 3.. if entity @s[tag=mode.ranged] run scoreboard players add @s ai_timer.3 1
    # run
        execute if score @s ai_timer.3 matches 200 facing entity @n[tag=target] feet anchored eyes positioned ^ ^ ^ run function asset:mob/skeleton/action/disgusting_worm/
    # reset score
...
# skull shot
    ...
    # caution
        execute if score @s ai_timer.4 matches 80 run function asset:mob/skeleton/action/skull_shot/caution
    # keep adding score
        execute if score @s ai_timer.4 matches 80.. run scoreboard players add @s ai_timer.4 1
    # run
        execute if score @s ai_timer.4 matches 100 run function asset:mob/skeleton/action/skull_shot/
    # reset score
...
# curse of deossification
    ...
    # caution
        execute if score @s ai_timer.5 matches 60 run function asset:mob/skeleton/action/curse_of_deossification/caution
    # keep adding score
        execute if score @s ai_timer.5 matches 60.. run scoreboard players add @s ai_timer.5 1
    # run
        execute if score @s ai_timer.5 matches 80 run function asset:mob/skeleton/action/curse_of_deossification/
    # reset score
