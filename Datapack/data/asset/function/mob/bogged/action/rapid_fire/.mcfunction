#> asset:mob/bogged/action/rapid_fire/
#
# shot arrow rapidly
#
...
# set data to summmoned arrow
    ...
    # set data
        # owner
            execute as @e[type=arrow,distance=..2,tag=rapid_fire.bogged.arrow,tag=!already_init] at @s run data modify entity @s Owner set from entity @n[tag=this] UUID
        # components
            execute as @e[type=arrow,distance=..2,tag=rapid_fire.bogged.arrow,tag=!already_init] at @s run data modify entity @s item.components set from entity @n[tag=this] equipment.offhand.components
        # weapon
            execute as @e[type=arrow,distance=..2,tag=rapid_fire.bogged.arrow,tag=!already_init] at @s run data modify entity @s weapon set from entity @n[tag=this] equipment.mainhand
    # add init tag
        tag @e[type=arrow,distance=..2,tag=rapid_fire.bogged.arrow,tag=!already_init] add already_init
