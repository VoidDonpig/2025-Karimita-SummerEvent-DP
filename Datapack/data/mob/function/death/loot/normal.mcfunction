...
# loot
    ...
    execute on attacker if entity @s[type=minecraft:player] run tag @s add this
    scoreboard players operation @p[tag=this] gold_queue += $gold_loot temporary
    execute if score $exp_loot temporary matches 1.. at @p[tag=this] run playsound minecraft:entity.experience_orb.pickup player @p[tag=this] ~ ~ ~ 1.0 2.0 0
